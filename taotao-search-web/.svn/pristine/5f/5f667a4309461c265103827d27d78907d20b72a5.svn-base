...
public class SendMail {
	public static void sendEmail(String subject, String text) throws MessagingException {
		...
		// 这里填你登录163邮箱所用的用户名
		msg.setFrom(new InternetAddress("shyw1995@163.com")); // 设置发件人，163邮箱要求发件人与登录用户必须一致（必填），其它邮箱不了解
		msg.setSubject(subject); // 设置邮件主题
		msg.setText(text); // 设置邮件内容
		Transport trans = session.getTransport();
		// 下面四个参数，前两个可以认为是固定的，不用变，后两个参数分别是登录163邮箱的用户名以及客户端授权密码（注意，不是登录密码）
		trans.connect("smtp.163.com", 25, "shyw1995@163.com", "wsy617510008"); // 连接邮箱smtp服务器，25为默认端口
		// 要发送到哪个邮箱，这里以qq邮箱为例
		trans.sendMessage(msg, new Address[] { new InternetAddress("872452093@qq.com") }); // 发送邮件
		trans.close(); // 关闭连接
	}
	...
	public static void groupSendEmail(String subject, String content) throws MessagingException {
		...
		Session session = Session.getInstance(props, new Authenticator() {
			protected PasswordAuthentication getPasswordAuthentication() {
				return new PasswordAuthentication("shyw1995@163.com", "wsy617510008");// 163邮箱用户名和客户端授权密码（注意，不是登录密码）
			}
		});
		...
		Message msg = new MimeMessage(session);
		// 邮件发送者
		msg.setFrom(new InternetAddress("shyw1995@163.com"));
		msg.setSubject(subject);
		// 注意第二个参数要写成"text/html;charset=utf-8"，表明这是一封html邮件
		msg.setContent(content, "text/html;charset=utf-8");
		// 要群发给哪些邮箱
		msg.setRecipients(RecipientType.TO, InternetAddress.parse("872452093@qq.com, 987824697@qq.com"));
